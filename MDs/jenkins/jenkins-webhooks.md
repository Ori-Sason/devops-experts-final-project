...
* A Webhook means that GitHub sends a request to our Jenkins instance, every time an event occurs (in our case, we will select Push and Pull Request (PR)).
* Install `Generic Webhook Trigger Plugin` plugin
  Jenkins → `Manage Jenkins` → `Plugins` → `Available plugins` → search for *Generic Webhook Trigger Plugin* → `Install`.
...
* **If you run jenkins on EC2 instance** - use the public IP.  
  ...
  **If you run Jenkins locally**, we need to expose Jenkins to the internet so GitHub will be able to send requests to it (Webhook). We can do that by using `Ngrok`. This tool creates a public URL that forwards request to a process running on your host machine.  
  ...
  * Since we expose our personal machine to the internet, we want to secure the connection and let **only** GitHub send requests to it. For that, create a YAML file (I named mine `github_policy.yml`).
    ...
    ```
  * Now we can open port 8080 to the internet
    ```bash
    ngrok http 8080 --traffic-policy-file ./github_policy.yml
    ```
    In the output you can see your personal link made by Ngrok, in `Forwarding` row (you can find the same link on your Dashboard in Ngrok). Every user has it's own static subdomain.
    ```bash
...
* Setup Webhooks on your repository  
  ...
  * `Events` - select `Let me select individual events` and choose 2 events: `Push` and `Pull Request`.
* Add the plugin settings at the top of Jenkinsfile (sets the trigger)  
  ...
  * You can see we have 2 secrets
    * The `Ngrok` secret (`this-is-my-G1tHub-little-secret`) is between GitHub and `Ngrok` (validates the request signature)
    * The Jenkins token (`secret-for-j3nk1ns-plugin`) is between `Ngrok` and Jenkins (identifies which job to trigger)
...
* There is an option to Notify GitHub that there is a job running related to a given event (like push or PR). We can do that by using `Pipeline GitHub Notify Step Plugin`.  
